 ...
 */
class DefaultStatusItemUIComponent : BaseStatusItemUIComponent<LinearLayout>() {
    ...
    override fun createContent(parent: LinearLayout) {
        (parent as? _LinearLayout)?.apply {
            ...
            space().lparams(matchParent, 0, 15.toFloat())

            imageView {
                id = VIEW_ID_ICON
                imageResource = R.drawable.ic_waring
                scaleType = ImageView.ScaleType.FIT_CENTER
            }.lparams(matchParent, 0, 20.toFloat())

            space().lparams(matchParent, 0, 5.toFloat())
